---
title: 'PPG Paint Colors: Final Project Classification'
...
From the plot above, we can see that AIC identifed model `glm_cat_lin` as the top performaer and BIC identified `glm_category` as the best model. The model `glm_all_linear` seems to have considerate low values of both BIC and AIC. Surprisingly, all the top 3 models has same accuracies.

Therefore the top 3 models are `glm_cat_lin`, `glm_all_linear` and `glm_category`. 
...
**How do the coefficient summaries compare between the top 3 models? Which inputs seem important?**

Most of the Saturation and Lightness levels in all the three plots are not zero and are significant. `glm_all_linear` is the only model with significant `R` values from the plot. 
...
It's the third best model with all the linear interactions between the inputs and has more statistical significant terms than the second best model. 
...
```
This large value suggests that, based on the evidence provided by the data, the model represented by `laplace_all_linear` is vastly preferred over the model represented by laplace_cat_lin. The higher the value, the stronger the evidence in favor of the model in the numerator.


```{r, posterior_coeff_viz_func}
...
Upon observing the predictive trends of the both models, it can be said that both the models are consistent with their own prediction and confidence intervals but they are not same across the models. 
...
As per the training set performance, `Gradient boosted tree` performed best followed by `RF` seemed to be the best.
...
**Overall, From the plot above, we can see that the model `XGB` seem to perform better overall followed by `RF` in terms of sensitivity and F1 score given the imbalances in the data.**
